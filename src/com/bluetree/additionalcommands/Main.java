	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (label.equalsIgnoreCase("additionalcommands")) {
			if (sender.hasPermission("additionalcommands.reload")) {
				if ((args.length >= 1) && (args[0].equalsIgnoreCase("reload"))) {
					...
					sender.sendMessage(
							ChatColor.translateAlternateColorCodes('&', "&bUsage : &c/additionalcommands reload"));
					return true;
				...
				;
			{
				sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
						this.getConfig().getString("default-no-permission-message")
				...
				getLogger().info(ChatColor.translateAlternateColorCodes('&',
						"&c" + sender.getName() + " was denied to access the command!"));
			}
		}
		...
		if (label.equalsIgnoreCase("website")) {
			if (sender.hasPermission(this.getConfig().getString("website.permission"))) {
				sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
						this.getConfig().getString("website.message")
								.replace("%plugin_version%", this.getDescription().getVersion()).replace("%player%",
										sender.getName().replace("%prefix%", this.getConfig().getString("prefix")))));

				return true;
			} else {
				sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
						this.getConfig().getString("website.no-permission-message")
								.replace("%plugin_version%", this.getDescription().getVersion()).replace("%player%",
										sender.getName().replace("%prefix%", this.getConfig().getString("prefix")))));
		...
		if (label.equalsIgnoreCase("discord")) {
			if (sender.hasPermission(this.getConfig().getString("discord.permission"))) {
				sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
						this.getConfig().getString("discord.message")
								.replace("%plugin_version%", this.getDescription().getVersion()).replace("%player%",
										sender.getName().replace("%prefix%", this.getConfig().getString("prefix")))));
				return true;
			} else
			...
			{
				sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
						this.getConfig().getString("discord.no-permission-message")
								.replace("%plugin_version%", this.getDescription().getVersion()).replace("%player%",
										sender.getName().replace("%prefix%", this.getConfig().getString("prefix")))));
		...
		if (label.equalsIgnoreCase("youtube")) {
			if (sender.hasPermission(this.getConfig().getString("youtube.permission"))) {
				sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
						this.getConfig().getString("youtube.meessage")
								.replace("%plugin_version%", this.getDescription().getVersion()).replace("%player%",
										sender.getName().replace("%prefix%", this.getConfig().getString("prefix")))));
				return true;
			} else
			...
			{
				sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
						this.getConfig().getString("youtube.no-permission-meessage")
								.replace("%plugin_version%", this.getDescription().getVersion())
								.replace("%player%", sender.getName())));
